...
if ! gh release view "$TAG" >/dev/null 2>&1; then
    ...
    gh release create "$TAG" --title "${KAM_MODULE_ID}-${KAM_MODULE_VERSIONCODE}-${KAM_MODULE_VERSION}" --notes-file "$TMP_CHANGELOG" $PRE_FLAG || log_warn "Failed to create release $TAG"
else
