...
- `KAM_TMPL_<NAME>`: Variables defined by a template in `[kam.tmpl.variables]` are exported as `KAM_TMPL_<NAME>` (upper-cased). The same variables are also available for template rendering as `{% raw %}{{ <name> }}{% endraw %}`.
