...
Kam 是一个为 Android 模块开发者设计的端到端工具链，支持 KernelSU、APatch、Magisk 和 AnyKernel3 模块工作流。它统一了从项目创建、本地开发、构建打包到真机联调的完整流程。
...
- **kam CLI**: 核心命令行工具，是与 Kam 生态系统交互的主要入口。它负责项目脚手架、模块构建、真机开发会话以及与测试环境的通信。
- **kamfw**: 一个纯 Shell 实现的轻量级模块框架。它为模块提供了标准化的生命周期、目录结构和基础功能接口，确保了模块在不同环境中的行为一致性。

## 🚀 快速上手

本章将引导你完成从安装 Kam CLI 到创建并构建第一个模块项目的过程。
