...
class PluginManager {
  ...
  async loadPluginFromConfig(config) {
    ...
    try {
      ...
      // 基于配置加载插件模块：优先使用 import.meta.glob 提供的导入映射，如果没有匹配则回退到动态导入（vite-ignore）
      const importPath = `../plugins/${config.id}/${config.entry || "index.js"}`;
      let pluginModule = null;

      // 如果在构建时由 import.meta.glob 收集到了插件映射，优先使用
      if (
        typeof __pluginImporters !== "undefined" &&
        __pluginImporters &&
        importPath in __pluginImporters
      ) {
        pluginModule = await __pluginImporters[importPath]();
      } else if (typeof import.meta !== "undefined" && import.meta.glob) {
        // 尝试使用 import.meta.glob 在运行时匹配（防止构建器扫描失败）
        const importers = import.meta.glob("../plugins/**/*.{js,ts,mjs}");
        if (importPath in importers) {
          pluginModule = await importers[importPath]();
        } else {
          // 回退：直接动态导入，不希望构建器跟踪此路径
  ...
  async _loadPluginLegacy(pluginId) {
    ...
    try {
      // 动态导入插件（优先使用 import.meta.glob 的预构建映射，如果没有则回退）
      const importPath = `../plugins/${pluginId}/index.js`;
      let pluginModule = null;
