...
impl KamModule {
    ...
    pub fn from_spec_and_toml(spec: &str, toml: KamToml) -> Result<Self> {
        let src = Source::parse(spec).map_err(|e| KamError::Other(format!("parse source spec: {}", e)))?;
    ...
    pub fn fetch_to_temp(&self) -> Result<PathBuf> {
        let src = match &self.source {
            Some(s) => s.clone(),
            None => return Err(KamError::Other("no source specified for module".to_string())),
        };
        ...
        match src {
            ...
            Source::Url { url } => {
                let tmp = tempdir()?;
                let resp = reqwest::blocking::get(&url).map_err(|e| KamError::Other(format!("failed to download {}: {}", url, e)))?;
                if !resp.status().is_success() {
                    return Err(KamError::Other(format!("download failed: {} -> {}", url, resp.status())));
                }

                let mut data = Vec::new();
                let mut reader = resp;
                reader.copy_to(&mut data).map_err(|e| KamError::Other(format!("read download body: {}", e)))?;
            ...
            Source::Git { url, rev } => {
                ...
                let repo = builder.clone(&url, tmp.path()).map_err(|e| KamError::Other(format!("git clone {}: {}", url, e)))?;

                if let Some(r) = rev {
                    let obj = repo.revparse_single(&r).map_err(|e| KamError::Other(format!("resolve rev {}: {}", r, e)))?;
                    repo.checkout_tree(&obj, None).map_err(|e| KamError::Other(format!("checkout tree: {}", e)))?;
                    repo.set_head_detached(obj.id()).map_err(|e| KamError::Other(format!("set HEAD: {}", e)))?;
                }
        ...
        let dest_name = if let Some(name) = self.canonical_cache_name() {
            ...
            match &self.source {
                ...
                Some(Source::Local { path }) => sanitize_name(&path.to_string_lossy()),
                    None => return Err(KamError::Other("no source available to derive name".to_string())),
            }
        };
...
fn extract_archive(path: &Path, dst: &Path) -> Result<()> {
    ...
    if s.ends_with(".zip") {
        ...
        return Err(KamError::Other(format!("unsupported archive format: {}", path.display())));
    }
    Ok(())
}
