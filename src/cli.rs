...
pub struct Cli {
    ...
    /// Pacman-style local package install flag (equivalent to pacman -U)
    ...
    /// Pacman-style cache clean modifier (use with -S as '-Sc')
    ...
    /// Pacman-style local package metadata query modifier (use with -Q as '-Qp')
    ...
    /// Install through adb for `kam install` or pacman-style `kam -U`.
    #[arg(long = "adb", action = clap::ArgAction::SetTrue, global = true)]
    pub adb: bool,

    /// Preferred root manager for install commands. Valid values: Auto, Magisk, KernelSU, APatchSU.
    #[arg(long = "manager", value_name = "MANAGER", global = true)]
    pub manager: Option<String>,

    /// Print install command plans without executing for commands that support it.
    #[arg(long = "dry-run", action = clap::ArgAction::SetTrue, global = true)]
    pub dry_run: bool,
