use super::cache::fetch_index_cached;
use super::download::fetch_module_detail;
...
pub fn search_remote(query: &str, base_url: &str) -> Result<(), KamError> {
    ...
    let scored = scored_results(query, base_url, &client, 0.60)?;
    if scored.is_empty() {
    ...
    for (i, (score, entry)) in scored.iter().take(50).enumerate() {
        print_search_result(base_url, &client, i, *score, entry, SearchPrintMode::Simple);
    }
    Ok(())
}
...
pub(crate) fn search_remote_interactive(
    ...
    let client = Client::builder()
        ...
        .build()
        .map_err(|e| KamError::FetchFailed(format!("Failed to build HTTP client: {e}")))?;
    let scored = scored_results(query, base_url, &client, 0.30)?;
    if scored.is_empty() {
        Utils::warn(trf!("repo.no_results_for", query));
        return Ok(None);
    }
...
fn print_search_result(
    ...
    if index < 5
        && let Ok(md) = fetch_module_detail(client, &entry.name, base_url)
    {
        print_release_metadata(&md);
    }
    println!();
}
...
fn print_release_metadata(md: &super::ModuleDetail) {
    if let Some(version) = latest_version(md) {
        println!("    {}: {version}", crate::i18n::tr("repo.version"));
    }
    if let Some(updated) = latest_updated(md) {
        println!("    {}: {updated}", crate::i18n::tr("repo.updated"));
    }
}

fn latest_version(md: &super::ModuleDetail) -> Option<&str> {
    md.latest_release.as_deref().or_else(|| {
        md.releases.as_ref()?.first()?.version.as_deref().or(md
            .releases
            .as_ref()?
            .first()?
            .name
...
fn latest_updated(md: &super::ModuleDetail) -> Option<&str> {
    md.latest_release_time.as_deref().or_else(|| {
        let first = md.releases.as_ref()?.first()?;
        first
            .published_at
            .as_deref()
