...
pub fn prepare_init(args: &super::InitArgs) -> Result<PreInitData, KamError> {
    ...
    let git_info = discover_git_info(&current_dir);

    // 默认分支名，如果无法获取则使用 "main"
    let default_branch = git_info.branch.as_deref().unwrap_or("main");

    // 智能推断项目名称：优先使用 git 仓库名，其次目录名
    let project_name_str = args.project_name.as_ref().map_or_else(
        || infer_project_name(project_name_raw, &current_dir, git_info.repo_name.as_deref()),
    ...
    let author = args.author.as_deref().map_or_else(
                || git_info.author.or(global_author).unwrap_or_else(|| "Your Name".to_string()),
        ToString::to_string,
    );
    ...
    if let Some(repo_url) = git_info.repo_url {
        // 从remote URL解析出owner/repo
        if let (Some(owner), Some(repo_name)) = (git_info.owner, git_info.repo_name) {
            ...
            if let Some(mmrl) = &mut kam_toml.mmrl {
                ...
                if let Some(repo) = &mut mmrl.repo {
                    ...
                    fill_repo_documentation_and_license(
                        repo,
                        &owner,
                        &repo_name,
                        default_branch,
