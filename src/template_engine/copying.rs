        ...
        for entry in WalkDir::new(src) {
            ...
            if entry.file_type().is_dir() {
                ...
                if is_binary(src_path) || !should_render_file(rel_path) {
                    ...
                    preserve_permissions(src_path, &dst_path)?;
                } else {
                    // 文本文件，尝试做模板替换
...
#[cfg(unix)]
fn preserve_permissions(src: &Path, dst: &Path) -> Result<(), KamError> {
    let mode = fs::metadata(src).map_err(KamError::Io)?.permissions().mode();
    let mut permissions = fs::metadata(dst).map_err(KamError::Io)?.permissions();
    permissions.set_mode(mode);
    fs::set_permissions(dst, permissions).map_err(KamError::Io)
}

#[cfg(not(unix))]
fn preserve_permissions(_src: &Path, _dst: &Path) -> Result<(), KamError> {
    Ok(())
}
