...
#[cfg(test)]
mod dedupe_tests {
    use super::dedupe_help;

    #[test]
    fn dedupe_usage_collapses_duplicate_tokens() {
        let raw = "Usage: kam install [OPTIONS] [PATH] [PATH]\n";
        let cleaned = dedupe_help(raw);
        assert!(cleaned.contains("Usage: kam install [OPTIONS] [PATH]\n"));
    }

    #[test]
    fn dedupe_arguments_and_options_sections_keep_last_block() {
        let raw = "Arguments:\n    description 1\n\nArguments:\n    description 2\n\nOptions:\n    -a    alpha\n\nOptions:\n    -a    alpha\n    -b    beta\n";
        let cleaned = dedupe_help(raw);
        assert!(cleaned.contains("description 2"));
